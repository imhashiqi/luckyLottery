...
let leftUsers = JSON.parse(localStorage.getItem("leftUsers")) ||[];

let awardList = JSON.parse(localStorage.getItem("awardList")) || {};


const user = leftUsers.sort(randomsort);

//不能说的秘密
const excludeUser = [["000005", "张无忌", "技术部", 0]]
/**
 * @description: 不能说的秘密
 * @param {*} nowItem 当前奖品
...
const EACH_COUNT = prizes.map(x => x.count);
export default { EACH_COUNT, prizes, COMPANY, user, luckyData, leftUsers, awardList, excludeUser, atmosphereGroupCard, background, setSecret, width, height, bgVideo, defaultUsers }