...
function initStyle(){
  if(mockData.bgVideo){
    bgVideo.innerHTML=`<video class="bg-video" src="${mockData.bgVideo}" loop="" muted="" autoplay=""></video>`
  }
...
function initCards() {
  ...
  {
    layer.alert('请添加人员后再刷新该页面');
    member = mockData.defaultUsers;
  }
...
function bindEvent() {
  document.querySelector("#menu").addEventListener("click", function (e) {
    ...
    if (!['reset', 'back', 'stop', 'import', 'import1', 'reload', 'import1'].includes(target)) {
      if (isLotting) {
  ...
  switch (type) {
    case "enter":
      ...
      default:
        ...
        btns.import.classList.add('none');
        btns.reload.classList.add('none');
...
function rotateBallNew(){
  scene.rotation.y = 0;
  console.log(scene.rotation);
  tween = new TWEEN.Tween(scene.rotation)
      .to(
        {
          y:Math.PI * 10000
        }, 19999)
...
function selectCard(duration = 600) {
  ...
  console.warn(selectedCardIndex);
  selectedCardIndex.forEach((cardIndex, index) => {
    changeCard(cardIndex, currentLuckys[index]);
    var object = threeDCards[cardIndex];
    console.warn(locates[index]);
...
function lottery() {
  ...
  rotateBall().then(() => {
    ...
    // })
    console.warn(basicData.leftUsers);
    currentLuckys = lotteryAn(perCount, basicData.leftUsers).map(index => {
        return cloneLeftUsers[index]
      });
...
function lotteryAn(time, leftUsers){
  var tmp = {};
  var tt = leftUsers.map((x,i) => Object.assign({
    4 : (Math.random() + x[3]),
    5 : i
  }, x))
  .sort((a, b) => b[4] - a[4])
  .slice(0, time)
  .map(x => x[5]);
  // for(var i = 0; i < cloneLeftUsers.length; i++){
  //   var current = cloneLeftUsers[i];
  //   current[4] = (Math.random() + current[3]);
  //   tmp[current[4]] = i;
  // }
  // var tt = Object.keys(tmp).sort((a, b) => b - a)
  //   .map(x => tmp[x]);
...
function shineCard() {
  ...
  setInterval(() => {
    ...
    for (let i = 0; i < shineCard; i++) {
      ...
      shine(cardIndex);
      var member = basicData.leftUsers.length == 0 ? mockData.defaultUsers : basicData.leftUsers;
      changeCard(cardIndex, member[index]);
    }
  }, 500);
}
